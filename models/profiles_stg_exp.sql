{{ config(
    ...
    depends_on=['profiles_stg'],
    on_schema_change='append_new_columns',
    pre_hook=[
        "{% if target.schema == 'dbt-dimensions' and source('dbt-dimensions', 'profiles_stg_exp') is not none %}TRUNCATE TABLE {{ source('dbt-dimensions', 'profiles_stg_exp') }};{% endif %}"
    ]
)}}
...
SELECT  
    ...
    final.walletprofileid,
    final.partnerid,
    final.hash_column,
    final.profile_createdat_local,
    final.profile_modifiedat_local,
    final.profile_deletedat_local,
    final.profile_type,
    final.partner_name_en,
    final.partner_name_ar,
    final.partner_isactive,
    final.partner_createdat_local,
    final.partner_modifiedat_local,
    final.partner_deletedat_local,
    final.utc,
    (now()::timestamptz AT TIME ZONE 'UTC' + INTERVAL '3 hours') AS loaddate

FROM {{ source('dbt-dimensions', 'profiles_stg') }} stg
JOIN {{ source('dbt-dimensions', 'profiles_dimension') }} final ON stg.walletprofileid = final.walletprofileid AND stg.profile_type = final.profile_type
